...
const ServicePermissionLayout = (props) => {
...
const mapState = (state) => {
    const { selectedServicePermission } = state.servicePermission;
    const { menuOpen } = state.layout;
    return { selectedServicePermission, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneServicePermission: (id) => dispatch.servicePermission.getOneServicePermission(id),
  });

export default connect(mapState, mapDispatch)(ServicePermissionLayout);

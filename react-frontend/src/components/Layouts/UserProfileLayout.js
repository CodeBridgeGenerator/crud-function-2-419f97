...
const mapState = (state) => {
    const { selectedUserProfile } = state.userProfile;
    const { menuOpen } = state.layout;
    return { selectedUserProfile, menuOpen };
  };
  const mapDispatch = (dispatch) => ({
    setMenuOpen: (bool) => dispatch.layout.setMenuOpen(bool),
    getOneUserProfile: (id) => dispatch.userProfile.getOneUserProfile(id),
  });

export default connect(mapState, mapDispatch)(UserProfileLayout);
